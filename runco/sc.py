...
class SlowControls(object):
    ...
    def query_timebinned_data(self,channel_id, dt_s, tstart, tstop,
        ...
        query = "SELECT channel_id, floor(extract(epoch from smpl_time)/%s) as tbin, avg(float_val) FROM sample WHERE smpl_time >= %s and smpl_time < %s and channel_id = %s and severity_id <= %s GROUP BY channel_id,tbin ORDER BY tbin;"
